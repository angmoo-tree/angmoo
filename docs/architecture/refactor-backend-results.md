...
### AR-B8-B Chat route 종료의 원본 검사 도입 기록

Signed source `68113382a2435faefdd7b65fd701d5ba081ec86b`의 변경 없는 `committed_snapshot` API 캡처를81.34초에 완료했다. 원래 tree `cbac80eedaf776e125d6661f17eb07e501f1e37d`·tracked2206파일·노드2709개와 전체 Git blob map을 검증했고, full/public 각196 API·schema 및102 ORM 계약이 불변 checkpoint와 완전히 같다. 원래203개 기록과 기존2698개 노드를 그대로 보존한 채 신규파일0·음성11개만204번째 기록으로 추가했다. 원래 checkpoint/addition production 검사의 오류0을 확인한 후보를 Host 실행 종료 후 반영했다. 이 커밋은 기록과 역사 WC package 설명만 변경하며, 실제 Host가 실행한68113382의 제품·테스트·검사기·CI·설치 source는 바뀌지 않는다. 최종204 원장의 공식 stock·전체 보안 및 원격/설치/post Gate는 별도 정확한 결과로 이어서 기록한다.

같은 clean68113382의 공식 Windows Host Tauri wrapper는 exit0으로 끝났다. Native Device Home에서 새 unclaimed owner 안내가 실제 표시됐고, Docker의 SQLite/Ladybug health 및 scheduler/projector ready를 확인했다. Alt+F4 뒤 새 창 목록이 비고 Angmoo/sidecar 프로세스가 없음을 확인했으며 wrapper의 설치 데이터 fingerprint·cleanup guards도 통과했다. 기존과 같은 비치명 WebView unregister1412 메시지는 따로 보존했다. 이 검증은 해당 Host 시작·화면·종료와 데이터 보호 범위이며 인증 완료 후 업무 기능이나 실제 AI 검증을 대신하지 않는다. 근거 로그는 `b8b-host-tauri-68113382.log`다.

### 2026-09-06 AR-B8-B 현재 Chat 읽기 지도 정정·exact589b 보안

첫 공식 `589b2e972ae709f4d3e9f4e2fcd906fadfd53b59` stock은 **protected2709=current2709**를 확인했으나, 삭제된 Chat route3의 current_paths가 K14에3개·K17/K18에각1개 남아 있어 **5개 경로 오류 / exit1**로 끝났다. 이 실패 로그를 `b8b-589b-original-stock.log`에 보존한다. 다른 원본 source/API/schema/ORM/assertion/suppression/node 오류는 없었다. 전체 mutable 현재 inventory를 다시 감사했고, 실제 canonical route들이 이미 같은 목록에 있으므로 `refactor_feature_inventory.json`의 current_paths5개와 K12/K14/K17/K18 current consumers12개에서 사용이 끝난 facade 항목만 제거했다. 원래 entrypoints·test_nodes·historical_transition_evidence·frontend·상태는 그대로다. 검색에서 남은 P8-A/B/P/Q artifact는 원래 successor의 frozen chain이며, 원래4개 검사도 통과했고 수정하지 않았다.

동일 exact589b의 보안 네 검사는 모두 PASS다: 실제 Gitleaks 현재/전체이력0건, custom 현재2206파일/검토11건/치명0 및 전체이력11485blobs/검토22건/치명0. 보안 검사의 exact SHA는589b이며 이후 변경은 위 현재 읽기 지도와 이 결과 문서뿐이다. 제품·테스트·검사기·CI·설치 source와204개 기록은 변경하지 않는다. 이 정정 후보의 원래 공식 stock을 다시 실행하며, 최종 원격 CI·설치·post Gate와 제품 PARTIAL은 별도로 유지한다.

### 2026-09-06 G01–G13 실제 소유·로컬 검증 현황 — 최종 Gate 미완료

전체 검증 기준 후보는 `30dcbd811a4c94aa7857c8ab8ceb4ac5b248c3d0`, 그 뒤 실제 Chat route facade 3개 제거를 고정한 소스는 `68113382a2435faefdd7b65fd701d5ba081ec86b`, 같은 소스의 원장·문서만 추가한 검증 기준은 `589b2e972ae709f4d3e9f4e2fcd906fadfd53b59`다. 아래는 실제 소유 코드와 로컬 검증 근거이며 최종 원격 CI·설치·병합 후 Gate를 대신하지 않는다. 30dc의 원래 전체 backend는 **2676 PASS / 22 SKIP / 28 warnings / 1087.30초**, 공식 stock은 **37항목 / protected 2698 = current 2698 / PASS**다. 로그는 `b8b-30dc-whole.log`, `b8b-30dc-whole.xml`, `b8b-original-stock.log`에 있다. 68113382의 Chat 제거는 별도 **60 PASS / 1 warning / 33.68초**, 원래 source/split/assertion/suppression/unrecorded 진단0과 경계1061/3961/legacy0으로 검증됐다. 원래203개 기록을 보존한204번째 기록에 새 음성11개를 추가했으며 원래 production checkpoint/addition 검사204 오류0이다. 현재 snapshot은2709개 노드다. 589b의 첫 공식 stock은 current_paths5개 오류로 exit1이었고 protected/current2709 및 나머지 보존 계약 오류는0이었다. 삭제된 facade가 남은 mutable 현재 목록17개를 정리한 metadata 후보에서 원래 stock을 다시 확인한다. Exact589b의 Gitleaks 현재/이력0·custom현재2206/치명0·이력11485/치명0을 확인했다. 30dc 전체 결과를 후속 변경의 새 전체 실행으로 부르지 않는다.

**공식 Host 범위 PASS:** clean68113382의 wrapper exit0, native Device Home의 새 unclaimed owner 안내 표시, Docker SQLite/Ladybug health 및 scheduler/projector ready, Alt+F4 후 새 창 목록/Angmoo·sidecar 프로세스 부재, 설치 데이터 fingerprint·cleanup guards를 확인했다. 기존 비치명 WebView unregister1412는 별도 기록했다. 이 결과는 인증 후 업무 기능이나 실제 AI 검증을 포함하지 않는다. 근거 `b8b-host-tauri-68113382.log`; 589b는 이 소스와 제품/test/checker/CI/installer 변화0이다.

| 항목 | 실제 최종 소유 및 유지 계약 | 직접 검증 근거 | 남은 종료 Gate |
| --- | --- | --- | --- |
| G01 설정 | `backend/app/config.py`가 실제 설정과 `BACKEND_DIR = Path(__file__).resolve().parents[1]`를 소유한다. backend `.env` 기준·기본 저장 경로를 유지한다. `runtime/configuration.py`는 명시적 설치 RuntimeConfig, `runtime/startup_security.py`는 시작 보안 판정이다. | `tests/config/test_config_paths.py`, `test_startup_security.py`: CWD와 독립적인 `.env`, 환경/저장/명시 설정 우선순위, 기본값·오류·cold import. 원래 전체 PASS에 포함. | 최종 설치 후보의 개발 `.env` 독립성과 환경 연결 확인. |
| G02 모델 | `app/models.py`가 유일한 Base다. `app/models/` package는 없다. `runtime/persistence/model_registration.py`가 23개 실제 도메인 모듈의 ORM 102개를 명시적으로 등록한다. `core/db.py`에는 지원 역사 migration용 같은 Base alias만 있다. | `tests/persistence/test_model_registration.py`: cold Base import의 DB/runtime 생성 없음, idempotent 등록, 102 mapper 같은 Base. 공식 ORM 계약 차이 0. | 최종 원격/설치의 동일 등록. 역사 alias 지원 조건은 별도로 유지. |
| G03 오류 | `app/exceptions.py`의 실제 공통 SQLite/concurrency/body-size/Agent 기반과 각 도메인의 실제 오류를 분리했다. 다른 오류를 이름이 비슷하다는 이유로 통합하지 않았다. | `tests/common/test_errors_pagination.py` 및 실제 HTTP 오류 회귀: 같은 class/args/reason, 413·503·409·status/detail 의미, 공식 API 계약 차이 0. | 최종 필수 CI. |
| G04 pagination | `app/pagination.py`는 공통 bytes URL-safe Base64 encode/decode만 소유한다. DeviceHome JSON cursor와 Social AESGCM profile cursor의 버전·scope·AAD·nonce·정렬·limit·query는 각각 실제 소유에 남는다. | `domains/device_home/repository.py`, `domains/social/service/profile_cursor.py`의 실제 소비; 공통 오류/pagination 회귀와 도메인 golden cursor 검증. | 최종 필수 CI. |
| G05 DB | `app/database.py`의 원래 DB 함수 6개와 engine/session cache·get_db 종료 의미를 유지한다. 앱 생성/요청/같은 Session 협력은 명시 등록과 실제 DB 모듈을 소비한다. | G5 원문 함수/504개 업무 정의 AST 동치, cold 44 PASS, 영향 195 PASS, G5 전체 2485 PASS/22 SKIP. B8-B 전체에서도 실제 Session/rollback/commit 회귀 포함. | 최종 설치 및 지원 실행 경로. |
| G06 앱 생성 | `app/main.py::create_app` 한 구현, `create_public_app = partial(create_app, profile="public")`, full/public export의 기존 기본값을 보존한다. contributor/sidecar가 실제 `main` public factory를 호출한다. `app/public_main.py`는 실제 없다. 로그 이름 `app.public_main`과 역사 param id는 활성 import가 아니다. | `tests/runtime/test_app_factory_ownership.py`, `test_canonical_bootstrap.py`, `test_public_factory_retirement.py`; 원래 signed G06/G5 provenance와 pure facade/actual factory/partial/default/ASGI 증명. full/public API 각각 196·ORM 102·ASGI 차이 0, 삭제 후보 cold/launcher 집중 및 전체 PASS. 실제 Host 삭제 후 실행·화면·종료도 위의 정확 범위 PASS. | §5.5의 최종 exact 후보 설치/필수 원격 CI·병합 후 검증은 부모가 진행 중. 최종 G06/B8 완료로 아직 승격하지 않는다. |
| G07 테스트 | 업무별 `tests/identity`, `characters`, `worlds`, `world_characters`, `world_packages`, `routines`, `routine_posts`, `social`, `relationships`, `chat`, `memory`, `runtime`, `local_bot`, `media` 등과 공통 `config/common/logging/migrations/persistence`로 실제 이전했다. 루트 횡단 검사는 남긴다. `conftest.py`, `fixtures`, model registration fixture를 보존한다. | old→new file/symbol/node map과 원본 assertion/suppression/source 검사가 기존2698개 전부를 보호한다. Chat route facade3을 제거하며 실제 Request DI의 원래 6 identity+4 type 조건과 원본 AST 증명을 유지했다. 새11개 음성 조건을 포함한60 PASS, 원본 signed source204번째 도입 기록의 production 검사0. | 최종204 원장의 공식 stock 및 필수 CI 수집. |
| G08 templates | `backend/templates/`가 없고 app/backend scripts의 Jinja2Templates/TemplateResponse 및 Jinja dependency 소비가 없다. 기능 없는 빈 폴더를 만들지 않는다. | 실제 source/import/dependency 검색. 기존 HTTP API는 JSON/stream 중심이며 공식 계약 보존. | 소비가 새로 생길 때만 해당 자원의 실제 owner/packaging 검증 필요; 현재 별도 생성 작업 없음. |
| G09 의존성 | `pyproject.toml`+`uv.lock`가 설치 기준이다. 별도 requirements 디렉터리를 만들지 않는다. Python `.python-version` 3.13·requires-python >=3.13, CI 고정 3.13.12 유지. | 원래 signed `19f084f4`의 역사 Alembic 검사 의존성 **dev pgvector==0.5.0**와 lock 항목만 추가됐으므로 lock이 #263과 byte 동일하다고 쓰지 않는다. Docker/CI는 frozen uv sync, sidecar PyInstaller 6.16.0의 실제 imports/자원 목록 유지. | 최종 CI frozen install 및 개발 도구 없는 설치 bundle. |
| G10 환경 | backend `.env.example`는 비밀 없는 예시이며 #263 blob 동일. 개발 `.env` 탐색과 설치 RuntimeConfig를 구분한다. | G01 설정 우선순위·설치 cold 회귀, 실제 현재/역사 secret 검사는 별도 보안 Gate에서 진행. | 최종 설치 및 exact 보안 결과. |
| G11 제외/패키징 | 저장소 root `.gitignore`, `.dockerignore`가 #263 blob 동일하다. `.env` 제외와 `.env.example` 포함 예외를 유지한다. Docker는 source/lock/README/Alembic/scripts/logging.ini, sidecar는 SQLite/Ladybug manifest·logging.ini를 포함한다. | `Dockerfile.backend`, `desktop/scripts/build-sidecar.ps1`; 기존 launcher/installer/embedded migration/Host 계약 검사. 불필요한 새 spec/requirements 파일 없음. | 실제 final build·설치·지원 실행 경로, 필수 원격 workflow·post. |
| G12 logging | `backend/logging.ini`, `app/runtime/logging_config.py`가 실제 source 또는 frozen `_MEIPASS` resource를 해석한다. CWD fallback 없음, 잘못된 설정 fail closed, factory는 기존 host handler/level 보존. sidecar의 없는 stdio/handshake 의미 유지. | `tests/logging/test_runtime_logging.py`의 handler·level·설정·sidecar health/shutdown, 전체 PASS. 원래 별도 rotation 구현은 없었으므로 rotation을 새로 구현했다고 표현하지 않는다. | 실제 최종 bundle의 로그 resource 로딩과 Host/설치 진단. |
| G13 Alembic | `backend/alembic/env.py`, `backend/alembic/versions/`, `alembic.ini`의 `%(here)s/alembic`/`%(here)s` 기준 및 현재 명시 ORM 등록·Base를 사용한다. 88 revision의 ID·내용·그래프와 head `20260904_0089` 유지. SQLite runtime version은 9다. | `tests/migrations/test_alembic_layout.py`와 실제 SQLite 지원 migration 검사. `b8b-g13-historical-byte-audit.json`: 역사 115개 중 111개 blob 동일, 나머지 4개는 정확 top-level owner import 변경만이며 비-import 전체 AST 동일. World scope migration 원문 byte 동일. | 최종 지원 PostgreSQL/SQLite/설치 경로의 필수 검증. 역사 alias는 지원 upgrade/downgrade 소비 종료 또는 별도 검증된 전환 전까지 유지. |

동일 Session 근거는 Identity 계정 삭제/credential transaction, Routines activity scope/policy/slot, Chat HTTP/response replay, Memory management/daypart binding 회귀와 각 signed source의 SQL/AST 증명에 있다. 단순 파일 이동이나 전체 숫자 하나만으로 이 계약을 대신하지 않는다.

현재 `legacy_exception_groups=[]`, `legacy_module_cycles=[]`, 실제 legacy import exceptions 0이다. **bridge는 38개이며 0이 아니다.** `b8b-bridge-current-audit.{json,md}`는 삭제 전41개를 감사한 역사적 근거다. 68113382에서 제품 소비가 끝난 Chat facade3을 제거해 역사 지원13개와 정상 runtime/HTTP 조립25개만 유지하고, 낡은 “다음 slice/G5 대기” 설명을 실제 지원 caller의 종료 조건으로 정리했다. 정상25개는 migrated owner의 역방향 import 차단도 유지하므로 bridge 수만 줄이려고 삭제하지 않는다. retained_modules는20개이며 지원 역사 migration와 외부 Hosted extension의 같은 객체 계약은 계속 보존한다. WC infrastructure package는 단순 빈 marker가 아니라 하위 역사 모듈 초기화에서 같은 CharacterActiveWorld/WorldCharacter class2개를 re-export한다.

이 표는 백엔드의 실제 구현·로컬 보존 증거를 정리한 것이다. 위의 공식 Host 시작/화면/종료 및 exact589b 보안은 확인됐지만 현재 지도 보정 뒤 최종 stock·보안의 metadata 대조·설치·원격 CI·병합 후 검증은 계속 진행 중이며, **제품 PARTIAL, §8.3·AR-X·P8-L-S 미완료/미착수 상태를 유지한다.** backend 종료를 frontend·제품 인과 루프·Release/Production 완료로 확대하지 않는다.
...
### Final PR #290: exact CI input corrections (2026-09-06)

The initial head `9ce4519d` retained two distinct CI failures. Local autonomy smoke reported **2 failed / 224 passed / 1 skipped / 28.42s**: the two original L3 boundary checks could not read the immutable signed source because their job used a shallow checkout. Only that job now requests `fetch-depth: 0`; its repository, exact source SHA, credentials setting, commands and protected assertions are unchanged. The original L3 and CI path checks passed **13 tests / 8.84s** after this input correction.

Security metadata validation and the full backend run both exposed the same stale expected tuple count. The full run completed **1 failed / 2686 passed / 22 skipped / 28 warnings / 289.65s**. The checker now requires exactly 26 tuples and separately verifies exactly one restored historical fixture path, its rule and its already reviewed fixture SHA256. Existing checkpoint, date, scope and duplicate constraints remain. The allowlist policy, scanner, product, tests and 204-record ledger are unchanged. The original metadata tests passed **12 tests** and isolated exact positive/negative controls passed **1 positive / 8 negative**; independent source review found no blocking issue. The new exact PR head must complete all required CI and installer gates; successful results from the initial head are historical evidence only.

Original failures: [Local autonomy](https://github.com/angmoo-tree/angmoo/actions/runs/34030424505/job/101478805432), [Security metadata](https://github.com/angmoo-tree/angmoo/actions/runs/34030424506/job/101478805356), [full backend](https://github.com/angmoo-tree/angmoo/actions/runs/34030424498/job/101478805578). Latest final PR/merge/post results remain linked from Issue264 and PR #290.
