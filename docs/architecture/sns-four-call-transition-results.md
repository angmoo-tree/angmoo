...
위 판정은 최초 구현 직후의 기록이다. 이후 사용자의 별도 전환 요청과 자연 활동 관찰 결과는 [9절](#9-후속-기본-전환과-자연-활동-관찰)에 시간 순으로 기록한다.
...
## 9. 후속 기본 전환과 자연 활동 관찰

최초 평가 뒤 사용자가 신규 personalized 활동부터 계약 2를 적용하도록 요청했다. 후속 커밋에서 `activity_engines.CONTRACT_VERSION`을 2로 바꾸고 로컬 Docker backend에 적용했다. 진행 중인 계약 1·2 활동은 각각 저장된 버전으로 재개하며 `current` 엔진의 계약 1 경로는 유지한다. 이 전환은 위 8쌍의 최초 구성요소 비교를 최종 스키마로 다시 수행했다는 뜻이 아니다.

이후 Routine에 World 현지 날짜·시각과 일과 구간을 계획/작성의 공통 입력으로 추가하고, 이전 게시글·상태 메모의 시간 표현이 현재 시각과 충돌할 때의 우선순위를 명시했다. 정상 통합 Routine 호출 수는 여전히 1회다. 실제 모델의 모든 서사 시간 정합성을 보장하거나 별도 의미 품질 평가를 완료했다고 판단하지 않는다.

2026-09-26 한국시간 14:38~16:38, 후속 구간 16:48까지의 관찰 세션 `sns-00f3e0b8c9e843139924e7b1a15ed76d`은 기록 누락·손상·heartbeat 공백 없이 `complete_recording=true`였다. 계약 2 활동 6건이 완료됐고 Inbox는 완료 3/무행동 3, Feed는 완료 3/무행동 3, Routine은 완료 6이었다. 공개 행동 12건이 성공했고 기록된 실행 오류와 Planner 출력 실패는 각각 0건이었다. 기억 조회 12건은 입력 예산에 따른 일부 생략을 포함해 `partial`로 기록됐다. 이 표본은 실행 안정성의 근거이며 캐릭터성·기억 활용·시간 서술 품질의 전체 PASS는 아니다.

후속 전환과 관찰의 상세 증거는 workspace의 `docs/plan/09-26 SNS 계약 버전 2 신규 활동 기본 전환과 Docker 적용 결과.md`, `docs/plan/09-26 Routine 현지 시각 정합성 구현과 로컬 검증 결과.md`, `.local-diagnostics/sns/sns-00f3e0b8c9e843139924e7b1a15ed76d/coverage.json`에 보존한다. 원본 진단·사용자 데이터는 제품 저장소에 포함하지 않는다.
