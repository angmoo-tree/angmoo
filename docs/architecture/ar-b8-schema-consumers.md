...
이 문서의 첫 소비자 전환 기준 source는 `9c14b6095b972bbbb1f02473725ea7a7983e7d6c`이다. 당시 실제 스키마 정의와 응답 계약을 바꾸지 않고 소비자가 업무 소유 파일을 직접 참조하도록 연결했으며, `app/schemas`의 여섯 호환 파일은 별도 검증을 위해 남겼다.

현재는 원래 import-only 본문과 모든 export의 실제 소유권을 검증하는 후속 작업으로 마지막 여섯 파일도 제거했다. 아래의 미완료·남은 검사 표현과 수치는 첫 소비자 전환 및 Literal 소유 이전 시점의 이력이다. 현재 구조와 원래 객체·업무 단언의 승계 근거는 [호환 집합 종료 기록](ar-b8-compatibility-retirement.md)을 따른다.
