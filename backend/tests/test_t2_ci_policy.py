...
def test_required_checks_include_embedded_data_migration_transition() -> None:
    assert "architecture-boundary" in checker.REQUIRED_JOBS
    assert "embedded-data-migration" in checker.REQUIRED_JOBS
    assert "sqlite-canonical-migration" in checker.REQUIRED_JOBS
    assert len(checker.REQUIRED_JOBS) == 11
