...
def test_guarded_recovery_uses_consumption_owner_and_preserves_admission(tmp_path, owner_controlled: bool) -> None:
    ...
    with Session(engine, expire_on_commit=False) as db:
        ...
        _add_social_event(db, event_id='guarded-expired-event', world_id=world.id, actor_world_character_id=other.world_character.id, target_world_character_id=fixture.world_character.id, occurred_at=now - timedelta(minutes=1))
        db.commit()
        plan = _prepare(db, fixture, now=now)
        episode = plan.items[0].episode
        assert episode is not None
        beat = activity_runtime.claim_activity_beat(db, episode_id=episode.id, scheduled_for=now, trigger_kind='comment_influenced', idempotency_key='guarded-beat', claim_run_id='guarded-run', claim_expires_at=now + timedelta(hours=1), source_event_ids=['guarded-expired-event'], now=now)
        consumption = activity_runtime.claim_event_consumption(db, references=SqlAlchemyActivityReferences(db), world_id=world.id, consumer_world_character_id=fixture.world_character.id, source_social_event_id='guarded-expired-event', target_activity_beat_id=beat.row.id, idempotency_key='guarded-consumption', claim_run_id='guarded-run', claim_expires_at=now + timedelta(minutes=5), now=now)
        beat_id = beat.row.id
    ...
    with Session(engine, expire_on_commit=False) as restarted:
        ...
        def recover():
            return _actual_domains_routines_service_lifecycle.recover_expired_claims(restarted, references=SqlAlchemyLifecycleReferences(restarted), clock=_actual_domains_routines_utils_clock.FrozenClock(now + timedelta(minutes=6)))
        if owner_controlled:
            with pytest.raises(_actual_domains_routines_exceptions.ActivityRuntimeValidationError, match='owner_controlled_automation_disabled'):
                recover()
            restarted.rollback()
        else:
            assert recover() == _actual_domains_routines_contracts_lifecycle.RecoveryCounts(beats=0, consumptions=1)
            assert recover() == _actual_domains_routines_contracts_lifecycle.RecoveryCounts(beats=0, consumptions=0)
