...
def _manifest(path: Path, *, sqlite=(1, 5, 5), ladybug=(0, 2, 2)) -> Path:
    identity_source = "\n".join(
        (
            "0.4.0-1",
            BUILD_COMMIT,
...
def test_installer_preflight_accepts_supported_active_generations(
    ...
    assert result.sqlite_target_version == 5
...
@pytest.mark.parametrize(
    ...
    (
        (6, 1, "installer_sqlite_data_incompatible"),
...
def test_installer_upgrade_mode_creates_current_generations_and_is_idempotent(
    ...
    assert second["sqlite_source_version"] == 5
