...
def test_l4_pr_a_architecture_and_parity_oracles_are_exact() -> None:
    ...
    assert backend["module_count"] == 586
    assert backend["internal_edge_count"] == 1381
    assert backend["external_import_count"] == 1907
    ...
    assert len(backend["ownership"]["canonical_boundaries"]) == 60
