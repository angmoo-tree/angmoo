...
def test_all_migrations_are_owned_exactly_once() -> None:
    ...
    assert inventory["migration_count"] == 83
    assert len(entries) == len({entry["path"] for entry in entries}) == 83
    assert len(entries) == len({entry["revision"] for entry in entries}) == 83
    assert all(
        entry["owner"] == "ER2"
...
def test_storage_frontend_runtime_and_parity_corpora_are_complete() -> None:
    ...
    assert frontend["route_count"] == 42
