...
def test_all_migrations_are_owned_exactly_once() -> None:
    ...
    assert inventory["migration_count"] == 85
    assert len(entries) == len({entry["path"] for entry in entries}) == 85
    assert len(entries) == len({entry["revision"] for entry in entries}) == 85
    assert all(
        entry["owner"] == "ER2"
