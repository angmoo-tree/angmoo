"""Shared deterministic prompt-safety policy.

The implementation is framework and provider independent.  P8-L-B gives the
policy a canonical non-service owner so the Chat runtime does not depend on a
legacy horizontal service module.
"""
