...
def signup(data: schemas.SignupCreate, db: Session = Depends(get_db)) -> schemas.AuthRead:
...
def demo_login(db: Session = Depends(get_db)) -> schemas.AuthRead:
    try:
        return auth_service.login_demo(db)
...
def logout(
    db: Session = Depends(get_db),
    session: models.AuthSession = Depends(get_current_session_for_logout),
) -> Response:
...
def complete_google_signup(
    ...
    _verify_turnstile(data.turnstile_token)
    try:
        return auth_service.complete_google_signup(db, data)
    except auth_service.InvalidGoogleSignupTokenError as exc:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
def link_google_account(
    ...
    try:
        return auth_service.link_google_account(db, user, data)
    except auth_service.GoogleAuthConfigError as exc:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Google login is not configured",
