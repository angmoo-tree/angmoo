...
class SqlAlchemyRunIdentityReferences:
    def __init__(self, db: Session, *, credential_lookup: Callable[[Session, str], LlmCredential | None], default_credential_lookup: Callable[..., LlmCredential | None]) -> None:
        self.db = db
        self._credential_lookup = credential_lookup
        self._default_credential_lookup = default_credential_lookup
    ...
    def character_suspended(self, reason: str) -> CharacterSuspendedError:
        return CharacterSuspendedError(reason)

    def get_default_credential(self, owner_id: str, *, character_id: str) -> LlmCredential | None:
        return self._default_credential_lookup(self.db, owner_id, character_id=character_id)
