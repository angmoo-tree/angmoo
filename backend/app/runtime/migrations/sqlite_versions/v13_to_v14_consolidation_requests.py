"""Add empty trigger receipts without reprocessing existing jobs or memories."""

from sqlalchemy import Connection
from app.runtime.migrations.sqlite_versions.contracts import SqliteMigrationDeltaError
from app.runtime.persistence.sqlite_schema import CONSOLIDATION_V14_TABLES

MUTABLE_IDENTITY_TABLES = frozenset(CONSOLIDATION_V14_TABLES)
...
def verify_delta(connection: Connection, before: frozenset[str]) -> None:
    if _inventory(connection) != before | MUTABLE_IDENTITY_TABLES:
        raise SqliteMigrationDeltaError("sqlite_migration_expected_delta_mismatch")
    for name in CONSOLIDATION_V14_TABLES:
...
def upgrade(connection: Connection) -> None:
    ...
    metadata.create_all(connection, tables=[metadata.tables[name] for name in CONSOLIDATION_V14_TABLES], checkfirst=False)
