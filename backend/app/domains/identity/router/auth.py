    ...
    try:
        return _browser_auth_response(
            ...
            auth_service.login(
                db,
                data,
                source=browser_session.request_source(request),
            ),
        )
...
def delete_me(
    ...
    response: Response,
    db: Session = Depends(get_db),
    workflow: AccountDeletionWorkflow = Depends(get_account_deletion_workflow),
    user: models.User = Depends(get_current_user_allow_incomplete),
) -> Response:
    try:
        auth_service.delete_current_user_account(db, user, data, workflow=workflow)
