...
export type ActivityRuntimeStatus = {
  ...
  runs: Array<{ activity_id: string; engine: string; status: string; stage: string;
    contract_version?: number; execution_order?: string[];
    paths: Record<string, { status: string; state_status: string | null; public_action_count: number; selected_count: number | null; recall_count: number | null; reason: string | null }>;
